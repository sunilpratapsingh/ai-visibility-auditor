...
function generateMarkdown(url, timestamp, scores, recommendations) {
  ...
  lines.push('---', '');
  lines.push('**Generated by [AI Visibility Auditor v2.1](https://sunilpratapsingh.com/audit?utm_source=ai-auditor&utm_medium=report-md&utm_campaign=v2.1)**');
  lines.push('Built on the Search Signal Framework by [Sunil Pratap Singh](https://sunilpratapsingh.com?utm_source=ai-auditor&utm_medium=report-md&utm_campaign=v2.1)');
  lines.push('');
  lines.push('> This audit checks if AI *can* see you. Want to know if AI *does* mention you?');
  lines.push('> [Get a professional AI visibility audit](https://sunilpratapsingh.com/consultant/geo?utm_source=ai-auditor&utm_medium=report-md-cta&utm_campaign=v2.1)');
  return lines.join('\n');
}
...
function renderDetailSitemap(data) {
  const body = document.getElementById('detail-sitemap-body');
  const s = data.sitemap;
  if (!s || !s.found) {
    body.innerHTML = ci('fail', 'No sitemap found. AI crawlers need sitemaps to discover your pages.');
    return;
  }
  let html = '';
  html += ci('pass', `<strong>Sitemap found</strong> (${esc(s.format || 'unknown')} format)`);
  if (s.urlCount > 0) html += ci(s.urlCount >= 10 ? 'pass' : 'warn', `<strong>${s.urlCount}</strong> URLs in sitemap`);
  if (s.lastmod) {
    html += ci(s.freshDays <= 30 ? 'pass' : 'warn', `Last modified: <strong>${esc(s.lastmod)}</strong> (${s.freshDays} days ago)`);
  } else {
    html += ci('warn', 'No lastmod dates in sitemap');
  }
  if (s.sources && s.sources.length > 0) {
    html += ci('info', `Found via: ${s.sources.map(esc).join(', ')}`);
  }
  body.innerHTML = html;
}

function renderDetailReadability(data) {
  const body = document.getElementById('detail-readability-body');
  const r = data.page.readability;
  if (!r) { body.innerHTML = ci('info', 'Readability data not available'); return; }
  let html = '';
  const ariLabel = r.ari <= 6 ? 'Easy' : r.ari <= 10 ? 'Optimal for AI' : r.ari <= 14 ? 'Complex' : 'Very Complex';
  html += ci(r.ari >= 6 && r.ari <= 12 ? 'pass' : r.ari < 6 ? 'warn' : 'fail',
    `Readability (ARI): <strong>${r.ari}</strong> — Grade ${r.gradeLevel} (${ariLabel})`);
  html += ci(r.avgSentenceLength >= 10 && r.avgSentenceLength <= 25 ? 'pass' : 'warn',
    `Avg sentence length: <strong>${r.avgSentenceLength}</strong> words`);
  html += ci(r.passiveVoicePercent <= 10 ? 'pass' : r.passiveVoicePercent <= 20 ? 'warn' : 'fail',
    `Passive voice: <strong>${r.passiveVoicePercent}%</strong> (${r.passiveCount} of ${r.sentenceCount} sentences)`);
  body.innerHTML = html;
}

function renderDetailPromotional(data) {
  const body = document.getElementById('detail-promotional-body');
  const p = data.page.promotional;
  if (!p) { body.innerHTML = ci('info', 'Promotional tone data not available'); return; }
  let html = '';
  const label = p.score >= 80 ? 'Non-promotional' : p.score >= 50 ? 'Mixed tone' : 'Promotional';
  html += ci(p.score >= 80 ? 'pass' : p.score >= 50 ? 'warn' : 'fail',
    `Promotional Tone Score: <strong>${p.score}/100</strong> (${label})`);
  html += ci(p.brandRatio <= 0.05 ? 'pass' : p.brandRatio <= 0.15 ? 'warn' : 'fail',
    `Brand-centric language: <strong>${Math.round(p.brandRatio * 100)}%</strong> (${p.brandCount} instances)`);
  html += ci(p.unsupportedSuperlatives === 0 ? 'pass' : p.unsupportedSuperlatives <= 3 ? 'warn' : 'fail',
    `Unsupported superlatives: <strong>${p.unsupportedSuperlatives}</strong>`);
  html += ci(p.ctaDensity <= 1 ? 'pass' : p.ctaDensity <= 3 ? 'warn' : 'fail',
    `CTA density: <strong>${p.ctaDensity.toFixed(1)}</strong> per 1000 words (${p.ctaCount} total)`);
  html += ci(p.comparativeCount === 0 ? 'pass' : 'warn',
    `Comparative self-promotion: <strong>${p.comparativeCount}</strong>`);
  if (p.score < 80) {
    html += ci('info', 'Research shows promotional tone has a -26% negative correlation with AI citation probability. Reduce brand-centric language and unsupported claims.');
  }
  body.innerHTML = html;
}
